...
var Managed_1 = require("../decorators/Managed");
var Store = /** @class */ (function () {
    ...
    Store.get = function (StoreConstructor) {
        var storeFound = Store.stores.get(StoreConstructor._storeIdentifier);
        if (!storeFound)
            throw new Error("Store used before its initialization. Check component hierarchy. Must be in boundaries of useStore which uses @Store.Manage decorator.");
        return storeFound.store;
    };
    Store.getAddRef = function (StoreConstructor) {
        var _a;
        var storeFound = (_a = Store.stores.get(StoreConstructor._storeIdentifier)) !== null && _a !== void 0 ? _a : {
    ...
    Store.removeRefDelete = function (StoreConstructor) {
        ...
        storeFound.refs--;
        if (!storeFound.refs)
            Store.stores.delete(StoreConstructor._storeIdentifier);
    };
    /*** @decorator* */
    Store.Managed = Managed_1.Managed;
    /**************
     * @statics
     * Dynamic store generation and removal by constructor arguments.
     *********** */
    Store.stores = new Map();
    return Store;
}());
exports.Store = Store;
