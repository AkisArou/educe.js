...
var Store = /** @class */ (function () {
    ...
    Store.getAddRef = function (StoreConstructor) {
        ...
        storeFound.refs++;
        Store.stores[StoreConstructor.name] = storeFound;
        console.log(__assign({}, Store.stores), "Store.stores getAddRef");
        return storeFound.store;
    };
    Store.removeRefDelete = function (StoreConstructor) {
        var storeFound = Store.stores[StoreConstructor.name];
        console.log(__assign({}, Store.stores), __assign({}, storeFound), "Store.stores removeRefDelete PRE --");
        storeFound.refs--;
        console.log(__assign({}, Store.stores), __assign({}, storeFound), "Store.stores removeRefDelete POST --");
        if (!storeFound.refs)
            delete Store.stores[StoreConstructor.name];
        console.log(__assign({}, Store.stores), __assign({}, storeFound), "Store.stores removeRefDelete DELETE");
    };
