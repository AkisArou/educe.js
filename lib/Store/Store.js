...
var Store = /** @class */ (function () {
    ...
    Store.get = function (StoreConstructor) {
        var storeFound = Store.stores.get(StoreConstructor);
        if (!storeFound)
            throw new Error("Store used before its initialization. Check component hierarchy. Must be in boundaries of useStore.");
        return storeFound.store;
    };
    Store.getAddRef = function (StoreConstructor) {
        var _a;
        var storeFound = (_a = Store.stores.get(StoreConstructor)) !== null && _a !== void 0 ? _a : {
    ...
    Store.removeRefDelete = function (StoreConstructor) {
        ...
        storeFound.refs--;
        if (!storeFound.refs)
            Store.stores.delete(StoreConstructor);
    };
    /**************
     * @statics
